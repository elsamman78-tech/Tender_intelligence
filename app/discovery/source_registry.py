...
from ..geography import is_target_country
...
def _is_social_domain(domain: str|None) -> bool:
    d=(domain or '').lower()
    return any(d==x or d.endswith('.'+x) for x in SOCIAL_DOMAINS)


def _source_type_for_domain(domain: str|None) -> tuple[str,int,int,str]:
    if _is_social_domain(domain):
        return 'BLOCKED_SOCIAL',0,0,'BLOCKED'
    return 'OPEN_WEB',30,50,'DISCOVERY'


def bootstrap_sources(db: Session) -> int:
    active_seed_names={seed['name'] for seed in SEED_SOURCES}
    # Retire seed-library portals removed from the current business scope, while
    # preserving their history for auditability.
    for old in db.scalars(select(Source).where(Source.discovered_by=='SEED_LIBRARY')).all():
        if old.name not in active_seed_names or (old.country and not is_target_country(old.country)):
            old.enabled=0
            old.lifecycle_status='OUT_OF_SCOPE'
    ...
    for seed in SEED_SOURCES:
        ...
        if not source:
            source=Source(
                ...
                requires_payment=0,discovered_by='SEED_LIBRARY',discovery_detail='Verified public seed source',enabled=1
            )
            db.add(source); db.flush(); added+=1
        else:
            source.enabled=1; source.lifecycle_status='ACTIVE'
            source.country=seed.get('country'); source.priority=seed['priority']; source.source_type=seed['source_type']
...
def get_or_create_discovered_source(db: Session, url: str, discovered_by: str, detail: str='') -> tuple[Source,bool]:
    ...
    if existing:
        if _is_social_domain(domain):
            existing.enabled=0; existing.lifecycle_status='REJECTED'; existing.source_type='BLOCKED_SOCIAL'; db.commit()
        return existing,False
    source_type,trust,discovery_value,purpose=_source_type_for_domain(domain)
    blocked=source_type=='BLOCKED_SOCIAL'
    src=Source(
        name=domain or url[:240],domain=domain or None,base_url=f'https://{domain}/' if domain else url,source_type=source_type,
        lifecycle_status='REJECTED' if blocked else 'CANDIDATE',priority='EXPERIMENTAL',trust_score=trust,relevance_score=0 if blocked else 30,
        discovery_value=discovery_value,health_status='BLOCKED' if blocked else 'UNKNOWN',cost_class='FREE_PUBLIC',requires_payment=0,
        discovered_by=discovered_by,discovery_detail=detail,enabled=0 if blocked else 1
    )
    db.add(src); db.flush()
    if not blocked:
