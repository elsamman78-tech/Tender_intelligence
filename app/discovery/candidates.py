...
def _is_saudi_db(text: str, country: str|None) -> bool:
    if normalize_country(country) != 'Saudi Arabia':
        return False
    low=(text or '').lower()
    return any(t in low for t in SAUDI_DB_TERMS) and (
        any(t in low for t in CONSULTANCY_TERMS) or
        any(t in low for t in ['design','engineering','bim','تصميم','هندسي','مخططات'])
    )


def _actionable_notice(text: str, country: str|None) -> bool:
    low=(text or '').lower()
    if opportunity_type(low) in {'AWARD','ADDENDUM'}:
        return False
    if _is_saudi_db(low,country):
        return True
...
def _extract_deadline(text: str):
    return _extract_date_near(text,[
        'deadline','closing date','submission deadline','bid closing','proposal due',
        'آخر موعد','الموعد النهائي','موعد تقديم العروض','إقفال المناقصة','اغلاق المناقصة'
    ])
...
def validate_candidate(db: Session, c: DiscoveryCandidate, auto_promote: bool=AUTO_PROMOTE_TENDERS):
    ...
    preliminary=(c.title or '')+' '+(c.snippet or '')
    saudi_db=_is_saudi_db(preliminary,c.country_guess)
    ...
    if result['hard_reject']:
        ...
        return {'status':'REJECTED','reason':c.rejection_reason}
    ...
    if participation['eligibility_status'] in {'NOT_ELIGIBLE_LANGUAGE','LOCAL_RESTRICTION'}:
        c.validation_status='REJECTED'; c.rejection_reason=participation['eligibility_status']; db.commit()
        return {'status':'REJECTED','reason':c.rejection_reason}
    ...
    if auto_promote:
        ...
        if not t:
            ...
            status='REVIEW_REQUIRED' if needs_review else 'QUALIFIED'
            t=Tender(
                ...
                tender_status=status,discovery_candidate_id=c.id,discovery_method=c.discovery_method,
                ...
                language_status=participation['language_status'],participation_notes=participation['notes'],
                source_evidence_type=_evidence_type(c,final_url),
